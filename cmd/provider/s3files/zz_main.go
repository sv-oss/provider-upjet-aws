...
func main() { //nolint:gocyclo // easier to follow as a unit
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-aws-s3files",
		Cache: cache.Options{
	...
	if startWebhooks {
		kingpin.FatalIfError(clustercontroller.SetupWebhookWithManager_s3files(mgr), "Cannot setup cluster-scoped webhooks")
		kingpin.FatalIfError(namespacedcontroller.SetupWebhookWithManager_s3files(mgr), "Cannot setup namespaced webhooks")
	}
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated_s3files(mgr, clusterOptions), "Cannot setup cluster-scoped AWS controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated_s3files(mgr, namespacedOptions), "Cannot setup namespaced AWS controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(clustercontroller.Setup_s3files(mgr, clusterOptions), "Cannot setup cluster-scoped AWS controllers")
		kingpin.FatalIfError(namespacedcontroller.Setup_s3files(mgr, namespacedOptions), "Cannot setup namespaced AWS controllers")
	}
